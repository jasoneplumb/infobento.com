...
Monorepo managed via npm workspaces: `packages/core` (types, layout), `packages/data` (box-data providers + cache), `packages/renderer` (2-bit grayscale framebuffer), `packages/api` (render API + auth/pairing over SQLite), `packages/web` (vanilla-TS config UI). Documentation in `docs/`.
