...
export function createPairHandler(getDb: () => DB) {
  return async (c: Context): Promise<Response> => {
    ...
    // (404) from "already owned by another account" (409). The lookup is the
    // only way to tell them apart — claimDevice returns null for both.
    const existing = getDeviceByPairCode(db, code);
    if (!existing) return c.json({ error: 'not_found' }, 404);

    const claimed = claimDevice(db, code, session.accountId);
    if (!claimed) return c.json({ error: 'already_claimed' }, 409);

    return c.json({ deviceId: claimed.id, config: parseConfig(claimed.config_json) });
  };
}
