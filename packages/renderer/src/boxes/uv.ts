...
function renderPlaceholder(
  ...
  // it has to be added to `y`. Assuming a single line overdraws "No data" on
  // top of a city name that wrapped, which is exactly what a multi-word city in
  // a narrow split box does; using the delta as an absolute Y paints it into a
  // different box entirely for any UV box not anchored at y = 0.
  let cy =
    y +
    drawTextWrapped(
      fb,
