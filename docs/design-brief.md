...
**Last reviewed:** 2026-08-23, against v0.38.3
...
- Share config JSON with recipient (export/import in the web editor), or claim the device to an account and save the layout server-side before gifting
...
Zero interaction remains the design ideal, but the shipped firmware keeps two deliberate controls: the green button flips orientation, and holding the two white buttons for 5s factory-resets. Nothing on the device configures content. The display charges via solar passively and refreshes on schedule; everything else happens in the web editor.
...
All box types use free data sources or local computation. Weather, forecasts, sunrise/sunset, air quality (AQI), UV, and pollen use Open-Meteo cloud data. Quotes use a quotable mirror (api.quotable.kurokeita.dev), stocks use Yahoo Finance, horoscope uses api-ninjas, on-this-day uses Wikipedia, and holidays use Nager.Date. The rest — QR, text, date, moon phase, countdown, and year progress — are pure local computation.
...
Alex opens infobento.com. The web UI shows a live eInk preview on the left with the editor on the right. They add Weather (type Portland -- geocoded via Nominatim), add Countdown (anniversary date), add a Date box, and a Quote box. The live preview updates after each change. They click Export Config and save the JSON as a backup. The config stays server-side: Alex signs in and claims the device by its Device ID, while the device's captive-portal setup takes only Wi-Fi credentials and that Device ID — no config is ever uploaded to the device.
...
Jordan buys an InfoBento for their friend who loves hiking. They configure 3 boxes: weather for the friend's city, a countdown to their next planned hike, and a QR code linking to their shared trail playlist. Jordan signs in, claims the device to an account by its Device ID, and saves the layout server-side, then includes a card with setup instructions. The friend opens the gift, connects to the device's captive portal, enters their Wi-Fi password and the Device ID from the sticker, and the display pulls Jordan's curated layout from the server within a minute.
...
- Up to 10 boxes filling the panel area (row cap derived from panel height and font size; minimum 4 rows)
...
Two-column layout at ≥1024px: preview on the left, editor on the right (stacking to a single column on narrow screens). Vanilla TS with reactive state, no framework.

**Elements:**

- Live eInk preview (server-rendered PNG, portrait + landscape); the simulator defaults to the reTerminal E1001 800×480 profile, with switchable resolutions
- Box list with up/down reordering, remove button, inline-editable labels
- Add-box chip palette (all 18 types, one click to add)
...
| ------------------------------------------------ | --------------------------- | --------------------- |
| Hardware dev kit validation (#57)                | Closed — validated on E1001 | Resolved              |
| Grey fallback -- Floyd-Steinberg dithering (#56) | Deferred                    | Grey proven on E1001  |
| Wire 5 new boxes into web UI (#53)               | Done — all 18 wired         | Resolved              |
| Content-aware layout for all types (#54)         | Shipped v0.13.0             | Resolved              |
| Price validation at volume                       | Needs quotes                | Kickstarter pricing   |
| SCAD enclosure model (#199, supersedes #50)      | Open                        | Prototype photography |
| Captive-portal firmware (#39)                    | Bench-verified              | Resolved              |
| Tilt switch hardware (#48)                       | Open                        | Auto-rotate           |

---

_InfoBento -- A calm bento dashboard for counters, desks, and shelves._
_Phase: Campaign | Authored June 2026 | Last reviewed 2026-08-23 against v0.38.3_
