...
module display_half() {
    difference() {
        ...
        // ESP32 recess (carved from one side, between panels)
        translate([esp_x_offset, esp_y_offset, display_half_thickness/2 - esp_depth/2])
            cube([esp_width, esp_height, esp_depth]);

        // Battery recess (carved from one side)
        translate([battery_x_offset, battery_y_offset, display_half_thickness/2 - battery_depth/2])
